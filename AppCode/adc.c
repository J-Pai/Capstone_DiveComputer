...
void adc_task(void * p_arg)
{
  ...
  for (;;)    
  {
    ...
    OS_MSG_SIZE size;
    uint32_t msg = (uint32_t)OSQPend(&g_adc_msg_queue, 0, OS_OPT_PEND_BLOCKING, &size, NULL, &err);
    my_assert(OS_ERR_NONE == err);

    if (msg == 0) {
      OSFlagPost(&g_alarm_flags, ALARM_NONE, OS_OPT_POST_FLAG_SET, &err);
      my_assert(OS_ERR_NONE == err);
    } else if (msg >= 1 && msg < 500) {
      OSFlagPost(&g_alarm_flags, ALARM_LOW, OS_OPT_POST_FLAG_SET, &err);
      my_assert(OS_ERR_NONE == err);
    } else if (msg >= 500 && msg < 624) {
